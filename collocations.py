...
output_path = r"C:\Users\Timur\OneDrive\Desktop\код\collocations_results.txt"

with open(output_path, "w", encoding="utf-8") as out:

    out.write("=" * 70 + "\n")
    out.write("биграмные коллокации (PMI) для целевых слов\n")
    out.write("=" * 70 + "\n")
    ...
    for target_lemma, original_word in sorted(target_lemmas.items(), key=lambda x: x[1]):
        ...
        if relevant_sorted:
            for w1, w2, score in relevant_sorted:
                ...
                out.write(f"    {w1} + {w2:<20} PMI = {score:.4f}   (партнёр: {partner})\n")
        else:
            out.write("    коллокации не найдены\n")

    out.write("\n" + "=" * 70 + "\n")
    out.write("триграмные коллокации (PMI) для целевых слов\n")
    out.write("=" * 70 + "\n")
    ...
    for target_lemma, original_word in sorted(target_lemmas.items(), key=lambda x: x[1]):
        ...
        if relevant_sorted:
            for w1, w2, w3, score in relevant_sorted:
                out.write(f"    {w1} + {w2} + {w3:<20} PMI = {score:.4f}\n")
        else:
            out.write("    коллокации не найдены\n")

    out.write("\n" + "=" * 70 + "\n")
    out.write("частотные биграммы (chi-square) для целевых слов\n")
    out.write("=" * 70 + "\n")
    ...
    for target_lemma, original_word in sorted(target_lemmas.items(), key=lambda x: x[1]):
        ...
        if relevant_sorted:
            for w1, w2, score in relevant_sorted:
                out.write(f"    {w1} + {w2:<20} chi2 = {score:.4f}\n")
        else:
            out.write("    коллокации не найдены\n")

    out.write("\n" + "=" * 70 + "\n")
    out.write("коллокации через spaCy (окно ±2 токена)\n")
    out.write("=" * 70 + "\n")
    ...
    for target_lemma, original_word in sorted(target_lemmas.items(), key=lambda x: x[1]):
        ...
        if top:
            for neighbor, count in top:
                out.write(f"    {neighbor:<25} частота = {count}\n")
        else:
            out.write("    коллокации не найдены\n")
